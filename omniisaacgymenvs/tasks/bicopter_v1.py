...
class Bicopter_v1Task(RLTask):
    def __init__(self, name, sim_config, env, offset=None) -> None:
        ...
        self._num_actions=5#动作空间：推力大小，旋转方向
        self._num_observations=21
        RLTask.__init__(self, name=name, env=env)

        self.max_thrust= 4.0#最大推力
    ...
    def pre_physics_step(self, actions):
        ...
        self.action=actions
        dof_x_action_speed_scale = 1 * math.pi
        dof_y_action_speed_scale = 1 * math.pi
        self.dof_position_targets[:, 0:2] += self.dt * dof_x_action_speed_scale * actions[:, 0:2]
        self.dof_position_targets[:, 2] += self.dt * dof_y_action_speed_scale * actions[:, 2]
        self.dof_position_targets[:, 3] += self.dt * dof_y_action_speed_scale * actions[:, 2]
        self.dof_position_targets[:] = tensor_clamp(
            self.dof_position_targets, self.dof_lower_limits, self.dof_upper_limits
        )
        thrust_action_speed_scale = 50*self.max_thrust
        self.thrusts += self.dt * thrust_action_speed_scale * actions[:, 3:5]
    ...
    def post_reset(self):
        ...
        self.action = torch.zeros((self._num_envs, 4), device=self._device, dtype=torch.float32)
        self.root_pos, self.root_rot = self._bicopter.get_world_poses(clone=False)
        self.root_velocities = self._bicopter.get_velocities(clone=False)
        self.dof_pos = self._bicopter.get_joint_positions(clone=False)
    ...
    def reset_idx(self, env_ids):
        ...
        self.dof_pos[env_ids, :] = torch_rand_float(0, 0, (num_resets, self._bicopter.num_dof), device=self._device)
        ...
        root_pos[env_ids, 2] += torch_rand_float(-0.2, 1, (num_resets, 1), device=self._device).view(-1)
    ...
    def calculate_metrics(self) -> None:
        ...
        # effort reward
        effort = torch.square(self.action).sum(-1)
        effort_reward = 0.05 * torch.exp(-0.5 * effort)

        dof_pos_re=(10*self.dof_pos*self.dof_pos)
        dof_pos_rew=torch.exp(-(dof_pos_re[:,0]+dof_pos_re[:,1]+dof_pos_re[:,2]+dof_pos_re[:,3]))

        speed = torch.square(root_speed).sum(-1)
        speed_reward = 0.2 * torch.exp(-1.0 * speed)
        # spin reward
        self.spin = torch.square(root_angvels).sum(-1)
        spin_reward =  torch.exp(-1.0 * self.spin)
        spinnage = torch.abs(root_angvels[..., 2])
        spinnage_reward = 1.0 / (1.0 + 1 * spinnage * spinnage)
        #spin_reward =  .0/(1.0+spin)
        # spinning
        #spinnage = torch.abs(root_angvels[..., 2])
        #spinnage_reward = 1.0 / (1.0 + 0.1 * spinnage * spinnage)

        rew = 2*pos_reward + pos_reward * (up_reward + spin_reward+spinnage_reward+spinnage * spinnage * (-1 / 400))-effort_reward
        self.rew_buf[:] = rew
    ...
    def is_done(self) -> None:
        ...
        #die = torch.where(self.spin > 50, ones, die)
