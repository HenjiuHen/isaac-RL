...
class my_cartpoleTask(RLTask):
    ...
    def set_up_scene(self,scene):#定义场景
        ...
        self._cartpoles=ArticulationView(#创建一个ArticulationView来保存环境集合??
            prim_paths_expr='/World/env/.*/Cartpole',name='cartpole_view',reset_xform_properties=False
        )
    ...
    def pre_physics_step(self, actions):#模拟步骤，执行动作和重置环境
        if not self._env_world.is_playing():#确保UI中的模拟没有停止
            return
        ...
        #计算动作力
        forces= torch.zeros((self._cartpoles.count,self._cartpoles.num_dof),dtype=torch.int32,device=self._device)#【智能体数量，每个智能体自由度个数】
